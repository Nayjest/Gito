...
Usage:
  CRACK deploy [--api-type TYPE] [--commit] [--rewrite] [--to-branch NAME]
...
def merge_gitlab_configs(
    ...
    new_include = {"local": ".gitlab/ci/CRACK-code-review.yml"}
    include_exists = any(
        (
            "CRACK-code-review.yml" in str(item.get("local", ""))
            if isinstance(item, dict)
            else "CRACK-code-review.yml" in str(item or "")
        )
...
GIT_PROVIDER_WORKFLOWS = {
    PlatformType.GITHUB: dict(
        code_review=dict(
            path=Path(".github/workflows/CRACK-code-review.yml"),
            template="workflows/github/CRACK-code-review.yml.j2",
        ),
        react_to_comments=dict(
            path=Path(".github/workflows/CRACK-react-to-comments.yml"),
            template="workflows/github/CRACK-react-to-comments.yml.j2",
        ),
    ),
    PlatformType.GITLAB: dict(
        code_review=dict(
            path=Path(".gitlab/ci/CRACK-code-review.yml"),
            template="workflows/gitlab/CRACK-code-review.yml.j2",
        ),
        gitlab_ci=dict(
...
@app.command(
    name="deploy",
    help="\bCreate and deploy CRACK workflows to your CI pipeline for automatic code reviews."
...
def deploy(
    ...
    to_branch: str = typer.Option(
        default="CRACK-ci", help="Branch name for new PR containing CRACK CI workflows"
    ),
    ...
    """Deploy CRACK to repository's CI pipeline for automatic code reviews."""
    print(logo())
    ...
    for file_config in workflow_files.values():
        ...
        if file.exists():
            message = f"CRACK CI workflow already exists at {utils.file_link(file)}"
            if rewrite:
                ui.warning(message)
            else:
    ...
    for key, file_config in workflow_files.items():
        ...
        if file.exists() and "merge_function" in file_config:
            ui.warning("Merging CRACK CI workflow into existing file:", utils.file_link(file))
    ...
    print(
        mc.ui.green("CRACK CI workflows have been created.\n"),
...
def _try_commit_workflow_changes(repo: Repo) -> bool:
    ...
    """
    try:
        repo.git.commit("-m", "Add CRACK CI workflows")
...
def _configure_llm(
    ...
    if not api_type:
        api_type = mc.ui.ask_choose(
            "Which language model API should CRACK use?",
            api_types,
        )
