"""Bootstrap module for initializing the CRACK application environment."""
...
from .constants import HOME_ENV_PATH, EXECUTABLE, PROJECT_CRACK_FOLDER, DEFAULT_MAX_CONCURRENT_TASKS
from .env import Env
...
def bootstrap(verbosity: int = 1):
    ...
    logging.info(
        f"Bootstrapping CRACK v{Env.CRACK_version}... " + mc.ui.gray(f"[verbosity={verbosity}]")
    )
    ...
    try:
        mc.configure(
            ...
            PROMPT_TEMPLATES_PATH=[PROJECT_CRACK_FOLDER, Path(__file__).parent / "tpl"],
        )
        ...
        if is_running_in_github_action():
            ...
            msg += (
                ...
                f"See https://github.com/Dishank422/CRACK/blob/main/documentation/github_setup.md "
                f"for details."
            )
